		...
		>
			...
			<div className="relative z-10 h-full w-full">
				...
				<Link href={card.link} target="_blank" rel="noopener noreferrer">
					<img src="/ExtendIcon.svg" alt="Extend" className="absolute bottom-12 right-4 w-7" />
				</Link>
				<button
					onClick={(e) => {
						e.stopPropagation();
				...
				>
					<img src="/InfoIcon.svg" alt="Info" className="absolute bottom-12 left-4 w-7" />
				</button>
				{infoCard === card.id && (
					<div className="absolute bottom-8 left-0 flex h-1/2 w-full flex-col items-center rounded-b-2xl bg-black p-4 transition-all duration-500 ease-in-out">
						...
						>
							<img src="/InfoIcon.svg" alt="Info" className="absolute bottom-4 left-4 w-7" />
						</button>
						<p className="mb-4 text-center text-white">{card.info}</p>
						<Link href={card.link} target="_blank" rel="noopener noreferrer">
							<img src="/ExtendIcon.svg" alt="Extend" className="absolute bottom-4 right-4 w-7" />
						</Link>
					</div>
				)}
			</div>
			...
			<div className="flex w-full justify-center" style={{ marginLeft: '10%' }}>
				<AnimatePresence>
					{inactiveCards.map((card, index) => (
						...
						>
							<ResourceCard card={card} infoCard={infoCard} handleInfoClick={handleInfoClick} />
						</motion.div>
					))}
				</AnimatePresence>
			</div>
