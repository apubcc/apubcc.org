...
export default function Faq() {
  return (
    <main className="flex justify-center gap-[5.6875rem]">
      <section>
        <hr className="bg-gradient-to-r from-[#F9C646] to-[#F8A02A] w-[11.875rem] h-[0.1875rem]"/>
        <h2 className="text-[2.375rem] font-black text-white">Frequently Asked Question</h2>
        <p className="text-white text-[1.3125rem] font-semibold">Answer to some question you might have</p>
      </section>
        ...
        <Accordion type="single" collapsible className="text-white">
          {
            faqItems.map((item, index) => (
              <AccordionItem key={index} value={String(index)}>
                <AccordionTrigger className="font-black text-[1.5rem]">
                  {item.title}
                </AccordionTrigger>
                <AccordionContent className="font-medium text-[1.25rem]">
                  {item.description}
                </AccordionContent>
              </AccordionItem>
            ))
